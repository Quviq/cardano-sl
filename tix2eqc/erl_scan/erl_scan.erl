...
string(String, StartLocation) ->
    real_string(String, StartLocation, []).

-spec real_string(String, StartLocation, Options) -> Return when
      String :: string(),
      Options :: options(),
      Return :: {'ok', Tokens :: tokens(), EndLocation}
              | {'error', ErrorInfo :: error_info(), ErrorLocation},
      ...
      EndLocation :: erl_anno:location(),
      ErrorLocation :: erl_anno:location().
real_string(String, Line, Options) when ?STRING(String), ?ALINE(Line) ->
    string1(String, options(Options), Line, no_col, []);
real_string(String, {Line,Column}, Options) when ?STRING(String),
                                            ?ALINE(Line),
                                            ?COLUMN(Column) ->
    string1(String, options(Options), Line, Column, []).

string(String, Pos, _) -> haskell_scan:string(String, Pos).
